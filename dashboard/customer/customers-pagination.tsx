...
export function CustomersPagination({ count, page, rows }: CustomersPaginationProps): React.JSX.Element {
  ...
  const updatePageParams = React.useCallback(
    (page: number): void => {
      ...
      navigate({ to: `${paths.dashboard.customers.list}?${searchParams.toString()}` });
    },
    [navigate]
  );
